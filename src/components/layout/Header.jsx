...
export default function Header() {
  ...
  const [exploreSubOpen, setExploreSubOpen] = useState(null);
  ...
  const handleMouseEnter = (megaType) => {
              ...
              <div className="w-60 border-r border-border p-4">
                {exploreCategories.map((cat) => (
                  ...
                  >
                    <Link
                      to={cat.to}
                      className={`flex items-center justify-between rounded-md px-3 py-2 text-sm transition-colors hover:text-primary ${exploreSubOpen === cat.label ? "text-primary" : "text-foreground"
                        }`}
                    >
                      {cat.label}
        ...
        <nav className="hidden lg:flex items-center gap-1 ml-8">
          {navLinks.map((link) => (
            ...
            >
              <Link
                to={link.to}
                className={`flex items-center gap-1 rounded-md px-3 py-2 text-sm font-medium transition-colors ${location.pathname === link.to
                  ? "text-accent"
